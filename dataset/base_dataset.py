...
class ImageVideoBaseDataset(Dataset):
    ...
    def load_and_transform_media_data_video(self, index):
        for i in range(self.num_tries):
            ann = self.anno_list[index]
            if self.sample_type == "key_frame":
                choosen_frames = self.choosen_key_frames[index]
            else:
                choosen_frames = self.choosen_uniform_frames[index]
            data_path = ann["image"]
            ...
            try:
                ...
                #     max_num_frames=max_num_frames, key_frames=key_frames,
                # )
                frames, frame_indices, video_duration = self.video_reader(
                    data_path, choosen_frames,
                )
