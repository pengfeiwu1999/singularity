...
def to_frame_indices(video_path, num_frames, sample='rand', fix_start=None, max_num_frames=-1, key_frames=[]):
    if sample == "key_frame":    # 新加的关键帧选取
        choosen_frame = [key_frames[random.randint(0, len(key_frames)-1)] for iter in range(num_frames)]
        return choosen_frame
    video_reader = decord.VideoReader(video_path, num_threads=1)
    vlen = len(video_reader)
    fps = video_reader.get_avg_fps()
    duration = vlen / float(fps)
...
def read_frames_fixed(video_path, choosen_frame_indices):
    video_reader = decord.VideoReader(video_path, num_threads=1)
    frames = video_reader.get_batch(choosen_frame_indices)  # (T, H, W, C), torch.uint8
    frames = frames.permute(0, 3, 1, 2)  # (T, C, H, W), torch.uint8
    vlen = len(video_reader)
    fps = video_reader.get_avg_fps()
    duration = vlen / float(fps)
    return frames, choosen_frame_indices, duration

VIDEO_READER_FUNCS = {
    'av': read_frames_av,
    'decord': read_frames_decord,
    'fixed_frame': read_frames_fixed
}
